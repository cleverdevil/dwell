...
authdb.sql('''create table if not exists codes (
    code varchar,
    client_id varchar,
...
def create_code(me, client_id, redirect_uri, state, response_type, scope, code,
                expires=None):
    '''
...
def auth_get():
    ...
    for arg in ('me', 'client_id', 'redirect_uri', 'state', 'response_type',
                'scope'):
...
def auth_post():
    ...
    response_type = flask.request.form['response_type']
