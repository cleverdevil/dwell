...
blueprint = flask.Blueprint(
    "site", __name__, template_folder=pathlib.Path("templates").absolute()
)
...
def photos():
    ...
    return flask.render_template(
        "photos.html", photos=photos, limit=limit, offset=offset, count=count
    )
...
def overview():
    ...
    kwargs = dict(
        ...
        blogs=model.Post.query(limit=30, kinds=["entry", "review", "recipe"]),
    )
...
def archive():
    ...
    result = model.database.connection.sql(
        " ".join(
            [
    ...
    return flask.render_template("archive/index.html", years=years)
...
def archive_day(year, month, day):
    ...
    return flask.render_template(
        "archive/day.html", posts=posts, year=year, month=month, day=day
    )
...
def content(kind, content_type):
    ...
    count = model.Post.query(
        count=True, kinds=[kind.__kind__.capitalize() for kind in kinds]
    )

    posts = model.Post.query(
        kinds=[kind.__kind__.capitalize() for kind in kinds], limit=limit, offset=offset
    )

    if content_type == "json":
        return {"type": ["h-feed"], "children": [post.json for post in posts]}

    flask.g.show_filters = True

    return flask.render_template(
        "feed.html", posts=posts, limit=limit, offset=offset, count=count
    )
...
def on_this_day(month, day):
    ...
    return flask.render_template(
        "on-this-day.html", posts=posts, date=date, month=month, day=day
    )
